...
# EasyEDA Copilot
Ассистент для EasyEDA Pro и JLCEDA на базе ИИ. Генерирует схемы по текстовому описанию, дорабатывает уже существующие цепи, ищет компоненты в каталоге LCSC по характеристикам, запускает SPICE-симуляцию и использует проверенные блоки схем в рамках рабочего процесса EDA.
...
- **Генерация схем по текстовому описанию**: опишите нужную схему и позвольте агенту собрать предложенный вариант.
- **Доработка существующих схем**: прикрепите выбранные фрагменты схемы и запросите у агента добавить, заменить или соединить нужные блоки.
...
- **Запуск SPICE-симуляции**: симулирует поддерживаемые схемы и проверяет выбранные модели, прежде чем доверять полученным результатам.
- **Проектирование печатных плат через MCP**: формирует размещение компонентов, собирает плату, выполняет трассировку, проверяет результат и запускает DRC через MCP-клиент.
...
5. Откройте схему и используйте `Copilot -> Interface`.

## MCP-сервер
EasyEDA Copilot может подключаться к внешним MCP-клиентам. Расширение сканирует `ws://127.0.0.1:8787` каждые 5 секунд и подключается, когда ваш MCP-сервер доступен. Элемент меню `Copilot -> MCP` приостанавливает и возобновляет это сканирование.
...
1. Добавьте MCP-сервер к вашему агенту.
2. Запустите Codex, Claude Code или другой MCP-клиент с включённым данным MCP-сервером.
3. Откройте схему в EasyEDA Pro.
4. EasyEDA Copilot автоматически подключится. Используйте Copilot -> MCP только для приостановки и возобновления сканирования

Читайте [README к пакету MCP](mcp/README.md)
