...
- выполнять DSL `eda-copilot-router` через управляемый backend KRT;
...
Типичный рабочий процесс: просмотр механического предварительного просмотра, утверждение окончательного размещения, импорт его в целевой документ PCB, затем `run_pcb_router_dsl` и проверка native DRC. Существующая медь сохраняется по умолчанию; полная или выборочная перетрассировка задаётся через `clearRouting(...)` в том же DSL. Долгие операции размещения и трассировки продолжаются единым инструментом `wait_operation`.
